...
const Event: React.FC = () => {
  const [query, setQuery] = useQueryParams({ page: StringParam, event_slug: StringParam, match: NumberParam });
  ...
  const handleCreateMatch = () => {
    ...
    setQuery({ page: 'match', event_slug: event.event_slug, match: event.matches.length });
  };
    ...
    <div className="flex flex-col items-center mt-8">
      ...
      <div className="w-full max-w-xl">
        ...
        <ul className="space-y-2">
          {event.matches.length === 0 && <li className="text-gray-500">No matches yet.</li>}
          {event.matches.map((_, idx) => (
            <li key={idx}>
              <button
                className="w-full text-left px-4 py-2 border rounded hover:bg-blue-50"
                onClick={() => setQuery({ page: 'match', event_slug: event.event_slug, match: idx })}
              >
                Match {idx + 1}
              </button>
            </li>
